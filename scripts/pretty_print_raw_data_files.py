...
def ensure_proper_date_range(file_names):
    ...
    for i in range(len(file_names)):
        from_date = parser.parse(file_names[i][0:4] + file_names[i][5:7] + file_names[i][8:10])
        to_date = parser.parse(file_names[i][14:18] + file_names[i][19:21] + file_names[i][22:24])
        assert to_date > from_date, f"Error: Found file with from date greater than to date {file_names[i]}"
    # next assert no gaps/overlaps in date ranges
    for i in range(len(file_names) - 1):
        from_date_next = parser.parse(file_names[i+1][0:4] + file_names[i+1][5:7] + file_names[i+1][8:10])
        to_date = parser.parse(file_names[i][14:18] + file_names[i][19:21] + file_names[i][22:24])
        assert from_date_next - to_date == datetime.timedelta(days=1), f"Error: Found gaps/overlaps in date range for  {file_names[i], file_names[i+1]}"
